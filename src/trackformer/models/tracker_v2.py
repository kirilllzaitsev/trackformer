...
class Tracker:
        ...
        if new_track_ids:
            ...
            if aux_results is not None:
                ...
                for new_track_id, aux_score in zip(new_track_ids, aux_scores):
                    self._logger.info(f"AUX SCORES ID {new_track_id}: {[f'{s:.2f}' for s in aux_score]}")
    ...
    def step(self, frame_data):
        ...
        orig_size = torch.stack(
            [
                frame_data["target"][i]["size"]
                for i in range(len(frame_data["target"]))
            ]
        )
        size = torch.tensor(img[0].shape[-2:]).unsqueeze(0).repeat(
            len(img), 1
        )
        self.inactive_tracks = [
            t for t in self.inactive_tracks
        ...
        # self._logger.info(f'FRAME: {self.frame_index + 1}')
        if self.inactive_tracks:
            self._logger.info(f'INACTIVE TRACK IDS: {[t.id for t in self.inactive_tracks]}')
        ...
        # self._prev_features is deque of size 1 containing only the t-1 features
        outputs, targets_res, features, _, _ = self.obj_detector(img, target, self._prev_features[0])

        hs_embeds = outputs['hs_embed'][0]

        results = self.obj_detector_post(outputs, orig_size)
        ...
        #         size.to(self.device),
        #         return_probs=True)
        # TODO: why taking the first batch? likely assumes bs=1
        result = results[0]
        ...
        if self._verbose:
            aux_results = [
                self.obj_detector_post(out, orig_size)[0]
        ...
        return outputs, targets_res, results, features
